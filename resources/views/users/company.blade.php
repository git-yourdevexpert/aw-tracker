            ...
            <div class="bg-white px-6 py-6 mt-6 rounded-md shadow-md text-gray-700 w-full">
                <div class="flex flex-wrap">
                    ...
                    <div class="mt-6 md:mt-0 w-full md:w-2/3">
                        <form action="{{ route('users.company.store') }}" method="POST" id="formCompany">
                            @csrf
...
@section('pageScript')
    <script>
        $('#formCompany').on('submit', function (e) {
            $('#btnCompany').attr('disabled', true).addClass('opacity-50');
            $('#btnCompany .spinner').removeClass('hidden');
            $('#btnCompany .btnText').text('Storing...');
        });
    </script>
@endsection
