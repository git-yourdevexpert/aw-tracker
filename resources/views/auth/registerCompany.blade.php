    ...
    <section class="px-6 py-6">
        <div class="container max-w-lg mx-auto flex-1 flex flex-col items-center justify-center px-2">
            <div class="bg-white px-6 py-8 rounded shadow-md text-black w-full">
                ...
                <form method="POST" action="{{ route('registerCompany.store') }}" id="formCompanyRegister">
                    @csrf
                    ...
                    <div>
                        ...
                        @foreach ($allProducts as $key => $product)
                            <input type="radio" name="product_id" id="product_id" value="{{ $product->id }}">
                            {{ $product->name }}<br />
                            Amount: ${{ $product->default_price->unit_amount / 100 }}<br />
                        @endforeach
                    </div>
